...
| `defaultTab` | `string` | `tabs[0].value` | Default selected tab value |
| `className` | `string` | - | Additional CSS classes |
| `urlParamName` | `string` | `"tab"` | URL parameter name for this tab group |
| `historyMode` | `"push" \| "replace"` | `"push"` | Control URL history behavior |
| `shallow` | `boolean` | `true` | Whether to use shallow routing |
| `onTabChange` | `(value: string) => void` | - | Callback when tab changes |
| `containerHeight` | `string` | `"calc(100vh-5rem)"` | Container height CSS value |
| `persistInUrl` | `boolean` | `true` | Whether to persist tab state in URL |
...
| `content` | `ReactNode` | - | Content to render in the tab panel |
| `disabled` | `boolean` | `false` | Whether the tab is disabled |
| `hidden` | `boolean` | `false` | Whether the tab is hidden |
